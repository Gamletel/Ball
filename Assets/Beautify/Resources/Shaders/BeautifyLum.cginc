	...
	half      _BlurScale;
		...
		sampler2D _BloomSourceTex;
		sampler2D_float _BloomSourceDepth;
		sampler2D _BloomSourceTexRightEye;
		sampler2D_float _BloomSourceDepthRightEye;
	...
	struct v2fLum {
		...
		#endif
		#if BEAUTIFY_BLOOM_USE_LAYER && UNITY_SINGLE_PASS_STEREO && defined(BEAUTIFY_DEPTH_BASED_SHARPEN) 
	...
	v2fCrossLum vertCrossLum(appdata v) {
		...
		UNITY_INITIALIZE_VERTEX_OUTPUT_STEREO(o);
		o.pos = UnityObjectToClipPos(v.vertex);
		#if BEAUTIFY_BLOOM_USE_LAYER && defined(BEAUTIFY_DEPTH_BASED_SHARPEN) && UNITY_SINGLE_PASS_STEREO
			o.depthUVNonStereo = v.texcoord;
			o.depthUV = UnityStereoScreenSpaceUVAdjust(v.texcoord, _MainTex_ST);
			#if UNITY_UV_STARTS_AT_TOP
				if (_MainTex_TexelSize.y < 0) {
   	...
   	half4 fragLumAntiflicker(v2fCrossLum i) : SV_Target {
		...
		c4 = clamp(c4, 0.0.xxxx, _BloomWeights2.zzzz);
			...
			#if BEAUTIFY_BLOOM_USE_DEPTH && defined(BEAUTIFY_DEPTH_BASED_SHARPEN)
				half depthAtten = 1.0 - depth01 * _BloomDepthThreshold;
				c1.rgb *= depthAtten;
				c2.rgb *= depthAtten;
				c3.rgb *= depthAtten;
				c4.rgb *= depthAtten;
